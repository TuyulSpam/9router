...
import { applyProviderThinkingDefault, extractThinking } from "../translator/concerns/thinkingUnified.js";
...
export async function handleChatCore({ body, modelInfo, credentials, log, onCredentialsRefreshed, onRequestSuccess, onDisconnect, clientRawRequest, connectionId, userAgent, apiKey, ccFilterNaming, rtkEnabled, headroomEnabled, headroomUrl, headroomCompressUserMessages, cavemanEnabled, cavemanLevel, ponytailEnabled, ponytailLevel, pxpipeEnabled, pxpipeMinChars, pxpipeTimeoutMs, pxpipeTransform, onPxpipeEvent, onTokenSaverEvent, sourceFormatOverride, providerThinking }) {
  ...
  // Execute request
  let providerResponse, providerUrl, providerHeaders, finalBody, retryTelemetry;
  try {
    ...
    retryTelemetry = result.retryTelemetry;
      ...
      if (newCredentials?.accessToken || newCredentials?.copilotToken) {
        ...
        try {
          ...
          if (retryResult.response.ok) {
            ...
            retryTelemetry = retryResult.retryTelemetry;
          }
  ...
  if (!providerResponse.ok) {
    ...
    saveRequestDetail(buildRequestDetail({
      ...
      response: {
        ...
        status: statusCode,
        thinking: null,
        ...(sanitizePreOutputRetryTelemetry(retryTelemetry) || {}),
      },
      pxpipe: pxpipeSummary,
      status: "error"
    })).catch(() => { });
  ...
  const sharedCtx = { provider, model, body, stream, translatedBody, finalBody, requestStartTime, connectionId, apiKey, clientRawRequest, onRequestSuccess, pxpipe: pxpipeSummary, reqTag, log, retryTelemetry };
